...
RHOAM_PROMETHEUS_ROUTE=$(echo "https://$(oc get route kafka-prometheus -n redhat-rhoam-observability -o=jsonpath='{.spec.host}')")
...
IDLE_QUERIES=(\
  ...
  "avg_over_time(sum(container_memory_working_set_bytes{namespace='redhat-rhoam-observability-operator', pod!='', container=''}) [15m:10s])/1024/1024"\
  ...
  "avg_over_time(namespace:container_cpu_usage:sum{namespace='redhat-rhoam-observability-operator'} [15m])"\
...
LOAD_QUERIES=(\
  ...
  "max_over_time(sum(container_memory_working_set_bytes{namespace='redhat-rhoam-observability-operator',container='', pod!=''}) [${testDuration}s:10s])/1024/1024"\
  ...
  "max_over_time(namespace:container_cpu_usage:sum{namespace='redhat-rhoam-observability-operator'} [${testDuration}s])"\
